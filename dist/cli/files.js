...
async function walkDirectory(root, current, output, unsupported) {
...
export async function collectInputFiles(inputPath, options = {}) {
    ...
    if (info.isDirectory()) {
        ...
        if (shouldIgnoreEntry(path.basename(absolute), false))
            throw new Error("No uploadable files found");
    ...
    if (unsupported.length > 0 && !options.ignoreUnsupported)
        throw new UnsupportedFilesError(unsupported);
    if (files.length === 0)
        throw new Error("No uploadable files found");
    ...
    if (!entrypoint)
        throw new Error("No HTML entrypoint found");
