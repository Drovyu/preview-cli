...
async function commentToken(key) {
...
async function storePreviewUrl(previewId, url) {
    const database = await openDatabase();
    await new Promise((resolve, reject) => {
        const transaction = database.transaction("previews", "readwrite");
        if (url)
            transaction.objectStore("previews").put(url, previewId);
        else
            transaction.objectStore("previews").delete(previewId);
...
async function start() {
    ...
    if (!/^[0-9a-f]{24}$/.test(previewId) || !/^[A-Za-z0-9_-]{22}$/.test(clientId) || !config?.vapidPublicKey || !config.previewDomain)
        throw new Error("通知設定リンクが正しくありません。");
    const previewUrl = new URL(previewUrlValue);
    const key = new URLSearchParams(previewUrl.hash.slice(1)).get("k") || "";
    if (!/^[A-Za-z0-9_-]{43}$/.test(key) || previewUrl.hostname !== `${previewId}.${config.previewDomain}`)
    ...
    async function request(action, subscription) {
        ...
        if (!response.ok)
            throw new Error(body.error || `通知設定に失敗しました (${response.status})`);
    ...
    disableButton.addEventListener("click", async () => {
        ...
        try {
            ...
            if (subscription)
                await request("unsubscribe", subscription);
