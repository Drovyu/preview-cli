...
import type { EncryptedManifest, ManifestFile, PreviewManifest } from "../shared.js";
...
import { buildCommand, type PackageJson, type SupportedBuilder } from "./build.js";
...
import {
  getLocalPreview,
  getLanguage,
  getOrCreateSupportStore,
  getSupportStore,
  type Language,
  listLocalPreviews,
  removeLocalPreview,
  removeLocalPreviewsByApiUrl,
  savePreview,
  saveSupportStore,
  setLanguage,
  uninstallLocalData
...
type MessageKey =
  ...
  | "previewCommand"
  | "recreateCommand"
  | "recreatePathArgument"
  | "recreateTargetArgument"
  | "remaining"
  | "removedLocalData"
  | "runBuild"
  | "ttlInvalid"
  ...
  | "updateCommand"
  | "updatePathArgument"
  | "updateTargetArgument"
  | "uploadPreparing"
  | "uploadUploaded"
  | "uploadUploading";

const messages: Record<Language, Record<MessageKey, string | ((...values: string[]) => string)>> = {
  en: {
...
function t(key: MessageKey, ...values: string[]): string {
  const message = messages[language][key];
  return typeof message === "function" ? message(...values) : message;
}

type HelpTextKey = "root" | "preview" | "create" | "update" | "recreate" | "list" | "delete" | "usage" | "lang" | "uninstall";

const helpTexts: Record<Language, Record<HelpTextKey, string>> = {
  en: {
    root: `
Common workflows:
...
function helpText(key: HelpTextKey): string {
  return helpTexts[language][key];
}

type EncryptedPreviewPayload = {
  sourcePath: string;
  files: Array<{ path: string; storageKey: string; bytes: Uint8Array; meta: ManifestFile }>;
  entrypoint: string;
  totalSize: number;
...
function parseTtl(value: string | undefined): number | undefined {
  ...
  if (!match?.[1] || !match[2]) throw new Error(t("ttlInvalid", value));
...
function printPreviewListItem(index: number, lines: Array<[string, string]>): void {
...
function createProgressBar(label: string, total: number): ProgressBar {
  let current = 0;
  const useTty = Boolean(process.stdout.isTTY);
  const width = 24;

  function render(itemLabel = ""): void {
    if (!useTty) {
...
async function ensureSupportCredentials(): Promise<NonNullable<Awaited<ReturnType<typeof getSupportStore>>>> {
  return getOrCreateSupportStore();
}

async function printSupportStatus(): Promise<void> {
  ...
  if (status.active) {
    ...
    if (status.expiresAt) console.log(`${t("expires")}: ${formatListDate(status.expiresAt)}`);
    console.log(language === "ja"
  ...
  if (status.pending) {
    ...
    if (credentials.claimCode) console.log(`${language === "ja" ? "支援コード" : "Claim code"}: ${credentials.claimCode}`);
    if (status.claimExpiresAt) console.log(`${t("expires")}: ${formatListDate(status.claimExpiresAt)}`);
    return;
  }
  console.log(language === "ja" ? "支援者特典は無効です" : "Supporter benefits are inactive");
}

async function startSupportClaim(): Promise<void> {
...
async function promptSupportRecoveryEmail(): Promise<string> {
...
async function recoverSupportClaim(): Promise<void> {
...
async function shouldIgnoreUnsupportedFiles(error: UnsupportedFilesError): Promise<boolean> {
...
async function collectPreviewInputFiles(inputPath: string): Promise<Awaited<ReturnType<typeof collectInputFiles>>> {
  try {
    return await collectInputFiles(inputPath);
  } catch (error) {
    if (!(error instanceof UnsupportedFilesError)) throw error;
    if (!await shouldIgnoreUnsupportedFiles(error)) throw new Error(t("uploadCancelled"));
    return collectInputFiles(inputPath, { ignoreUnsupported: true });
  }
}

async function importEncryptionKey(keyString: string): Promise<CryptoKey> {
  if (!/^[A-Za-z0-9_-]{43}$/.test(keyString)) throw new Error("Invalid local encryption key");
  const raw = base64UrlDecode(keyString);
  if (raw.byteLength !== 32) throw new Error("Invalid local encryption key");
  const copy = new Uint8Array(raw.byteLength);
  copy.set(raw);
  return crypto.subtle.importKey("raw", copy.buffer as ArrayBuffer, "AES-GCM", false, ["encrypt"]);
}

async function prepareEncryptedPreview(inputPath: string, key: CryptoKey): Promise<EncryptedPreviewPayload> {
  ...
  const encryptedFiles: Array<{ path: string; storageKey: string; bytes: Uint8Array; meta: ManifestFile }> = [];
  let totalSize = 0;
  let encryptedSize = 0;
...
async function uploadEncryptedPreview(apiUrl: string, id: string, uploadId: string, key: CryptoKey, payload: EncryptedPreviewPayload): Promise<void> {
  ...
  const encryptedManifest: EncryptedManifest = {
    version: 1,
...
async function requireSupporterOptions(apiUrl: string, options: PreviewOptions): Promise<void> {
  if (!options.permanence && options.ttlSeconds === undefined) return;
  const credentials = await getSupportStore();
  if (!credentials) {
    throw new Error(language === "ja"
...
async function createEncryptedPreview(inputPath: string, apiUrl = getApiUrl(), options: PreviewOptions = {}): Promise<{ id: string; url: string }> {
...
async function updateEncryptedPreview(saved: NonNullable<Awaited<ReturnType<typeof getLocalPreview>>>, inputPath: string, apiUrl = saved.apiUrl, options: PreviewOptions = {}): Promise<{ id: string; url: string }> {
  await requireSupporterOptions(apiUrl, options);
...
async function resolveCreateInput(inputPath: string | undefined): Promise<string> {
  if (inputPath) return path.resolve(inputPath);
  const distPath = path.resolve("dist");
  if (await pathExists(distPath)) return distPath;
  const indexPath = path.resolve("index.html");
  if (await pathExists(indexPath)) return indexPath;
  throw new Error(t("noInput"));
}

async function resolveLocalPreviewTarget(target: string | undefined, inputPath: string | undefined): Promise<{ saved: NonNullable<Awaited<ReturnType<typeof getLocalPreview>>>; sourcePath: string }> {
...
function detectBuilder(pkg: PackageJson): SupportedBuilder | undefined {
  ...
  if ("astro" in dependencies || /\bastro\s+build\b/.test(buildScript)) return "astro";
  if ("vite" in dependencies || /\bvite\s+build\b/.test(buildScript)) return "vite";
  return undefined;
}

function defaultOutDir(builder: SupportedBuilder): string {
...
async function runBuild(projectPath: string, packageManager: string, builder: SupportedBuilder, pkg: PackageJson, outDir: string): Promise<void> {
...
async function replacePreview(id: string, sourcePath: string, apiUrl: string, options: PreviewOptions = {}): Promise<{ id: string; url: string; replaced: boolean }> {
  await requireSupporterOptions(apiUrl, options);
...
async function createOrRecreatePreview(sourcePath: string, apiUrl = getApiUrl(), options: PreviewOptions = {}): Promise<{ id: string; url: string; replaced: boolean }> {
  const resolvedSourcePath = path.resolve(sourcePath);
  const local = await listLocalPreviews();
  const saved = local.find((item) => item.apiUrl === apiUrl && item.sourcePath === resolvedSourcePath);
  if (saved) return replacePreview(saved.id, resolvedSourcePath, apiUrl, options);
  const created = await createEncryptedPreview(resolvedSourcePath, apiUrl, options);
  return { ...created, replaced: false };
}

function registerBuildPreviewCommand(): void {
  program
    .command("preview")
...
program
  ...
  .action(async (scope, options) => {
    if (scope && scope !== "all") throw new Error(t("listScopeInvalid", scope));
    ...
    for (const saved of visibleLocal) {
      ...
      const lifetime: [string, string] = preview?.retention_mode === "permanent"
      ...
      if (saved.sourcePath) lines.push([t("entrySource"), saved.sourcePath]);
  ...
  .action(async (value) => {
    ...
    const nextLanguage = normalized === "ja" || normalized === "japanese"
      ...
      : normalized === "en" || normalized === "english"
        ? "en"
        : undefined;
    if (!nextLanguage) throw new Error(t("langInvalid", value));
...
supporterCommand
  ...
  .action(async (recoveryToken) => {
    if (!SUPPORT_TOKEN_PATTERN.test(recoveryToken)) throw new Error(language === "ja" ? "recovery tokenが正しくありません" : "Invalid recovery token");
    const credentials = await ensureSupportCredentials();
...
supporterCommand
  ...
  .action(async () => {
    const credentials = await getSupportStore();
    if (!credentials) throw new Error(language === "ja" ? "支援者設定がありません" : "Supporter setup has not started");
