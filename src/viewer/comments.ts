...
type DecryptedComment = EncryptedComment & { payload: CommentPayload };

declare global {
  interface Window {
    __DVYU_COMMENTS_CONFIG__?: CommentsConfig;
    __dvyCommentsLoaded?: boolean;
    __dvyCommentsLoading?: boolean;
  }
}
...
async function startCommentsWithRetry(configValue: CommentsConfig): Promise<void> {
...
async function getPreviewKey(configValue: CommentsConfig): Promise<{ key: string; root: boolean }> {
  if (configValue.outer) {
    const key = new URLSearchParams(location.hash.slice(1)).get("k");
    if (key && /^[A-Za-z0-9_-]{43}$/.test(key)) return { key, root: true };
    throw new Error("プレビューキーが見つかりません");
  }
  return new Promise<{ key: string; root: boolean }>((resolve, reject) => {
    ...
    function onMessage(event: MessageEvent) {
      if (event.origin !== location.origin || event.data?.type !== "DVY_COMMENT_KEY" || event.data?.id !== configValue.previewId) return;
      if (typeof event.data.key !== "string" || !/^[A-Za-z0-9_-]{43}$/.test(event.data.key)) return;
...
async function commentToken(keyString: string): Promise<string> {
...
function currentRoute(configValue: CommentsConfig): string {
  if (configValue.outer) return "expired";
  const route = new URL(location.href);
  route.searchParams.delete("__dvy_sw_retry");
  return `${route.pathname}${route.search}${route.hash}`;
}

function startRouteSync(configValue: CommentsConfig): void {
...
function storedNotificationClientId(): string {
  ...
  try { localStorage.setItem(storageKey, created); } catch {}
  return created;
}

function outerNotificationClientId(previewId: string): Promise<string> {
  return new Promise((resolve) => {
    ...
    function receive(event: MessageEvent) {
      if (event.origin !== location.origin || event.source !== window.parent || event.data?.type !== "DVY_NOTIFICATION_CLIENT" || event.data?.id !== previewId) return;
      if (typeof event.data.clientId !== "string" || !/^[A-Za-z0-9_-]{22}$/.test(event.data.clientId)) return;
      clearTimeout(timeout);
      window.removeEventListener("message", receive);
...
function quoteFor(element: Element | null): string | null {
  const text = element?.textContent?.replace(/\s+/g, " ").trim();
  return text ? text.slice(0, 240) : null;
}

async function startComments(configValue: CommentsConfig): Promise<boolean> {
  let keyResult: { key: string; root: boolean };
  try {
    keyResult = await getPreviewKey(configValue);
  } catch {
    return false;
  }
  if (!keyResult.root) return false;
  const keyString = keyResult.key;
  if (!configValue.outer) startRouteSync(configValue);
  ...
  const launcherCount = root.querySelector<HTMLElement>(".launcher-count")!;
  const menu = root.querySelector<HTMLElement>(".menu")!;
  const workspace = root.querySelector<HTMLElement>(".workspace")!;
  const capture = root.querySelector<HTMLElement>(".capture")!;
  const selection = root.querySelector<HTMLElement>(".selection")!;
  const draftsLayer = root.querySelector<HTMLElement>(".drafts")!;
  const composer = root.querySelector<HTMLElement>(".composer")!;
  const textarea = root.querySelector<HTMLTextAreaElement>("textarea")!;
  const nameDialog = root.querySelector<HTMLElement>(".name-dialog")!;
  ...
  const commentsContainer = root.querySelector<HTMLElement>(".comments")!;
  const commentPopover = root.querySelector<HTMLElement>(".comment-popover")!;
  const popoverAvatar = root.querySelector<HTMLElement>(".popover-avatar")!;
  const popoverAuthor = root.querySelector<HTMLElement>(".popover-author")!;
  const popoverMeta = root.querySelector<HTMLElement>(".popover-meta")!;
  const popoverText = root.querySelector<HTMLElement>(".popover-text")!;
  const popoverReplies = root.querySelector<HTMLElement>(".popover-replies")!;
  const replyInput = root.querySelector<HTMLTextAreaElement>(".reply-composer textarea")!;
  const replySend = root.querySelector<HTMLButtonElement>(".reply-send")!;
  const drafts: Array<{ payload: CommentPayload; commentId: string }> = [];
  const renderedAnchors: Array<{ comment: DecryptedComment; pin: HTMLElement; region?: HTMLElement }> = [];
  let pendingAnchor: RouteAnchor | null = null;
  let dragStart: { x: number; y: number } | null = null;
  let activeCommentItem: HTMLElement | null = null;
  let activeCommentPin: HTMLElement | null = null;
  let popoverLocked = false;
  let popoverHideTimer = 0;
  let activeRootComment: DecryptedComment | null = null;
  ...
  if (usesOuterToolbar) workspace.classList.add("external-toolbar");

  if (configValue.expired) root.querySelector<HTMLElement>('[data-action="add"]')!.hidden = true;
  ...
  async function refreshCommentCount(): Promise<void> {
    try {
      ...
      const page = await response.json() as { count?: number };
      if (Number.isFinite(page.count)) updateCommentCount(Number(page.count));
    } catch {
  ...
  function saveDisplayName(value: string): void {
    ...
    try {
      if (normalized === "匿名") localStorage.removeItem(displayNameStorageKey);
      else localStorage.setItem(displayNameStorageKey, normalized);
    } catch {
  ...
  if (usesOuterToolbar) {
    ...
    try { displayName = normalizeDisplayName(localStorage.getItem(displayNameStorageKey)); } catch {}
  }

  function updateSendButton(): void {
    sendButton.textContent = `送信 ${drafts.length}`;
    sendButton.disabled = drafts.length === 0;
    if (usesOuterToolbar) window.parent.postMessage({ type: "DVY_COMMENT_DRAFTS", id: configValue.previewId, count: drafts.length }, location.origin);
  }

  function resolveAnchorRect(anchor: RouteAnchor, route: string): { left: number; top: number; width: number; height: number } | null {
    if (!configValue.outer && route !== currentRoute(configValue)) return null;
    if (anchor.selector) {
      try {
  ...
  function positionCommentAnchors(): void {
    for (const rendered of renderedAnchors) {
      ...
      if (rendered.region) rendered.region.hidden = !visible;
      if (!rect) continue;
      if (rendered.region) Object.assign(rendered.region.style, { left: `${rect.left}px`, top: `${rect.top}px`, width: `${rect.width}px`, height: `${rect.height}px` });
  ...
  function renderDrafts(): void {
    ...
    drafts.forEach((draft, index) => {
      ...
      while (occupiedLabels.some((item) => labelRect.left < item.right + 6 && labelRect.right + 6 > item.left && labelRect.top < item.bottom + 6 && labelRect.bottom + 6 > item.top) && attempts < drafts.length) {
        ...
        if (labelTop + toolbarOffset + labelRect.height > innerHeight - 12) labelTop = Math.max(4, labelTop - (labelRect.height + 6) * 2);
        label.style.top = `${labelTop}px`;
        labelRect = label.getBoundingClientRect();
        attempts += 1;
      }
  ...
  let positionFrame = 0;
  function scheduleAnchorPosition(): void {
  ...
  function exitAddMode(): void {
    ...
    if (usesOuterToolbar) window.parent.postMessage({ type: "DVY_COMMENT_MODE", id: configValue.previewId, active: false }, location.origin);
  }
  ...
  function formatRelativeTime(value: string): string {
    ...
    if (elapsed < 2_592_000_000) return `${Math.floor(elapsed / 86_400_000)}日前`;
    return new Date(value).toLocaleDateString();
  }

  function schedulePopoverClose(): void {
    clearTimeout(popoverHideTimer);
    if (popoverLocked) return;
    popoverHideTimer = window.setTimeout(closeCommentPopover, 120);
  }

  function showCommentPopover(comment: DecryptedComment, replies: DecryptedComment[], index: number, pin: HTMLElement, item: HTMLElement, expanded: boolean): void {
    clearTimeout(popoverHideTimer);
    ...
    if (left + popoverRect.width > innerWidth - 12) left = pinRect.left - popoverRect.width - 10;
  ...
  root.querySelector('[data-action="add"]')!.addEventListener("click", () => {
    ...
    renderDrafts();
    updateSendButton();
    if (usesOuterToolbar) window.parent.postMessage({ type: "DVY_COMMENT_MODE", id: configValue.previewId, active: true }, location.origin);
  });
  ...
  root.querySelector('[data-action="notifications"]')!.addEventListener("click", async () => {
    ...
    try {
      if (window.top?.location.href) previewUrl = window.top.location.href;
    } catch {}
  ...
  root.querySelector<HTMLElement>(".popover-close")!.addEventListener("click", closeCommentPopover);
  ...
  root.addEventListener("click", (event) => {
    ...
    if (popoverLocked && !commentPopover.contains(target) && !target.closest(".pin")) closeCommentPopover();
  });
  ...
  if (usesOuterToolbar) {
    addEventListener("message", (event) => {
      if (event.origin !== location.origin || event.source !== window.parent || event.data?.id !== configValue.previewId) return;
      if (event.data.type === "DVY_COMMENT_NAME") {
        displayName = normalizeDisplayName(typeof event.data.name === "string" ? event.data.name : "");
        if (!nameDialog.hidden) nameInput.value = displayName === "匿名" ? "" : displayName;
        return;
      }
      if (event.data.type !== "DVY_COMMENT_COMMAND") return;
      if (event.data.command === "close") exitAddMode();
      if (event.data.command === "clear") clearDrafts();
      if (event.data.command === "send") sendButton.click();
    });
  }

  capture.addEventListener("pointerdown", (event) => {
    if (event.button !== 0 || !composer.hidden) return;
  ...
  capture.addEventListener("pointerup", (event) => {
    ...
    const type: RouteAnchor["type"] = distance < 8 ? "point" : "region";
  ...
  root.querySelector('[data-action="stage"]')!.addEventListener("click", () => {
    ...
    if (!text || !pendingAnchor || drafts.length >= 25) return;
    drafts.push({
      commentId: randomId(),
  ...
  async function postEncryptedComments(entries: Array<{ commentId: string; payload: CommentPayload }>): Promise<void> {
    ...
    if (!response.ok) throw new Error(`コメントを送信できませんでした (${response.status})`);
    const result = await response.json() as { count?: number };
    if (Number.isFinite(result.count)) updateCommentCount(Number(result.count));
  }
  ...
  sendButton.addEventListener("click", async () => {
    ...
    if (usesOuterToolbar) window.parent.postMessage({ type: "DVY_COMMENT_DRAFTS", id: configValue.previewId, count: drafts.length, sending: true }, location.origin);
    try {
  ...
  async function decryptComment(comment: EncryptedComment): Promise<DecryptedComment | null> {
    try {
      const aad = new TextEncoder().encode(`dvyu-comment-v1:${configValue.previewId}:${comment.commentId}`);
      const plaintext = await crypto.subtle.decrypt(
        { name: "AES-GCM", iv: Uint8Array.from(base64UrlDecode(comment.iv)).buffer, additionalData: aad },
        cryptoKey,
        Uint8Array.from(base64UrlDecode(comment.ciphertext)).buffer
      );
      const payload = JSON.parse(new TextDecoder().decode(plaintext)) as CommentPayload;
  ...
  async function loadComments(): Promise<DecryptedComment[]> {
    ...
    do {
      ...
      if (!response.ok) throw new Error(`コメントを取得できませんでした (${response.status})`);
      const page = await response.json() as { comments: EncryptedComment[]; nextCursor: string | null; count?: number };
      if (Number.isFinite(page.count)) updateCommentCount(Number(page.count));
      encrypted.push(...page.comments);
      cursor = page.nextCursor;
    } while (cursor && encrypted.length < 10_000);
    return (await Promise.all(encrypted.map(decryptComment))).filter((comment): comment is DecryptedComment => comment !== null);
  }

  function anchorRect(comment: DecryptedComment): { left: number; top: number; width: number; height: number } | null {
  ...
  function focusComment(comment: DecryptedComment, replies: DecryptedComment[], index: number, pin: HTMLElement, item: HTMLElement): void {
  ...
  function navigateToComment(comment: DecryptedComment, replies: DecryptedComment[], index: number, pin: HTMLElement | null, item: HTMLElement): void {
    if (configValue.outer) return;
    let target: URL;
    try {
    ...
    try { sessionStorage.setItem(focusStorageKey, comment.commentId); } catch {}
    location.assign(targetRoute);
  }

  async function showComments(reopenCommentId?: string): Promise<void> {
    closeCommentPopover();
    panel.hidden = false;
    pins.hidden = false;
    ...
    try {
      ...
      if (!comments.length) commentsContainer.innerHTML = `<div class="empty">コメントはありません</div>`;
      ...
      comments.forEach((comment) => {
        if (!comment.payload.parentId || !ids.has(comment.payload.parentId)) return;
        const replies = repliesByParent.get(comment.payload.parentId) || [];
        replies.push(comment);
        repliesByParent.set(comment.payload.parentId, replies);
      });
      let reopenTarget: { comment: DecryptedComment; replies: DecryptedComment[]; index: number; pin: HTMLElement; item: HTMLElement } | null = null;
      roots.forEach((comment, index) => {
        ...
        if (comment.commentId === reopenCommentId) reopenTarget = { comment, replies, index, pin, item };
      });
      positionCommentAnchors();
      if (reopenCommentId) {
        try { sessionStorage.removeItem(focusStorageKey); } catch {}
      }
      if (reopenTarget) {
        const target = reopenTarget as { comment: DecryptedComment; replies: DecryptedComment[]; index: number; pin: HTMLElement; item: HTMLElement };
  ...
  try { pendingFocusCommentId = sessionStorage.getItem(focusStorageKey) || ""; } catch {}
  if (configValue.expired || pendingFocusCommentId) void showComments(pendingFocusCommentId || undefined);
