...
ブラウザ側のViewer、復号Service Workerの参照出力、実際に配信するViewer JSも検証用に
公開します。範囲と限界は[Viewerの公開範囲と信頼境界](docs/VIEWER.ja.md)を参照してください。
...
コメントメニューの「通知を設定」から、そのプレビューのブラウザ通知を有効にできます。通知許可は専用の`notify.dvyu.link`で管理します。通知には新着件数だけを含め、コメント本文、表示名、復号キーは含めません。復号キーを含む完全URLは通知設定を行ったブラウザのIndexedDBだけに保存され、Push購読情報、preview ID、ランダムなブラウザIDだけがサーバーへ送信されます。同じブラウザから送信したコメントはそのブラウザへ通知しません。別のブラウザや端末から送信した場合は、アカウントなしでは同一人物と判定できないため通知されます。
